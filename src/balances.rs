...
impl<B, C, T, U> FromRef<AppState<B, C, T, U>> for BalanceState
where
...
fn upsert_balance(
    ...
    let balance = connection
        ...
        .query_row(&[(":account", account)], map_row)?;
...
mod upsert_balance_tests {
    ...
    use crate::balances::{Balance, create_balance_table, upsert_balance};
    ...
    async fn can_upsert_balance() {
        ...
        let got = upsert_balance(&want.account, want.balance, &want.date, &connection)
